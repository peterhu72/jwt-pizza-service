...
orderRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    let p = await DB.getMenu();
    if (p.find((item) => item.id === orderReq.items[0].menuId).price !== orderReq.items[0].price){
    ...
    if (r.ok) {
      ...
      metrics.addRevenue(order.items.reduce((sum, item) => sum + item.price, 0));
    } else {
